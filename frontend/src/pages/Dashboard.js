...
const styles = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .d-root{min-height:100vh;background:#05080F;font-family:'DM Sans',sans-serif;color:#E2E4EF;overflow-x:hidden}
  .nav{position:sticky;top:0;z-index:200;background:rgba(5,8,15,0.92);backdrop-filter:blur(28px);border-bottom:1px solid rgba(255,255,255,0.05);height:66px;padding:0 32px;display:flex;align-items:center;gap:16px}
  ...
  .nav-search{width:100%;padding:9px 14px 9px 40px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:10px;color:#E2E4EF;font-size:14px;outline:none;transition:all 0.2s}
  .nav-search:focus{border-color:rgba(108,99,255,0.5);background:rgba(108,99,255,0.06)}
  .nav-search::placeholder{color:#374151}
  ...
  .nav-btn{padding:7px 16px;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer;transition:all 0.18s;border:1px solid rgba(255,255,255,0.09);background:transparent;color:#9CA3AF;font-family:'DM Sans',sans-serif}
  ...
  .nav-btn-chat:hover{background:#7C73FF}
  .nav-btn-out{border-color:rgba(239,68,68,0.25);color:#EF4444}
  .nav-btn-out:hover{background:rgba(239,68,68,0.08)}
  ...
  .hero-banner{position:relative;overflow:hidden;background:#080B16;border-bottom:1px solid rgba(255,255,255,0.04);padding:0}
  ...
  .hero-img-side img{width:100%;height:100%;object-fit:cover;opacity:0.35;filter:saturate(0.6)}
  .hero-img-center{overflow:hidden;position:relative;display:flex;align-items:center;justify-content:center}
  .hero-img-center img{width:100%;height:100%;object-fit:cover;opacity:0.25;filter:saturate(0.5)}
  .hero-overlay{position:absolute;inset:0;background:linear-gradient(135deg,rgba(108,99,255,0.15),rgba(167,139,250,0.08));display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:24px}
  .hero-tag{display:inline-flex;align-items:center;gap:7px;background:rgba(108,99,255,0.12);border:1px solid rgba(108,99,255,0.25);border-radius:99px;padding:5px 14px;font-size:12px;color:#A78BFA;font-weight:600;margin-bottom:16px}
  ...
  .hero-title{font-family:'Syne',sans-serif;font-size:42px;font-weight:800;color:#fff;letter-spacing:-2px;line-height:1.1;margin-bottom:12px;text-shadow:0 2px 30px rgba(0,0,0,0.8)}
  ...
  .hero-sub{color:#9CA3AF;font-size:15px;line-height:1.7;max-width:480px;text-shadow:0 1px 10px rgba(0,0,0,0.9)}
  .stats{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;max-width:1280px;margin:32px auto 0;padding:0 40px}
  .stat{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.055);border-radius:16px;padding:22px 24px;position:relative;overflow:hidden}
  .stat::after{content:'';position:absolute;inset:0 0 auto 0;height:1px;background:linear-gradient(90deg,transparent,rgba(108,99,255,0.45),transparent)}
  .stat-label{font-size:11px;font-weight:600;color:#4B5563;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px}
  .stat-val{font-family:'Syne',sans-serif;font-size:36px;font-weight:800;color:#fff;line-height:1;margin-bottom:6px}
  .hint-g{font-size:12px;color:#4ADE80}
  .hint-y{font-size:12px;color:#EAB308}
  .hint-b{font-size:12px;color:#60A5FA}
  ...
  .section-title{font-family:'Syne',sans-serif;font-size:22px;font-weight:700;color:#fff;letter-spacing:-0.4px}
  .section-sub{font-size:13px;color:#6B7280;margin-top:3px}
  .found-count{font-size:13px;color:#4B5563;align-self:center}
  .skills-box{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:20px;padding:28px}
  .inputs-row{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:20px}
  .f-label{font-size:10px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:1px;display:block;margin-bottom:7px}
  .s-inp{width:100%;padding:12px 16px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:10px;color:#E2E4EF;font-family:'DM Sans',sans-serif;font-size:14px;outline:none;transition:all 0.2s}
  .s-inp:focus{border-color:rgba(108,99,255,0.45);background:rgba(108,99,255,0.05)}
  .s-inp::placeholder{color:#2D3148}
  ...
  .alert-ok{background:rgba(74,222,128,0.07);border:1px solid rgba(74,222,128,0.18);border-radius:8px;padding:9px 13px;color:#4ADE80;font-size:13px;margin-bottom:14px}
  .alert-err{background:rgba(239,68,68,0.07);border:1px solid rgba(239,68,68,0.18);border-radius:8px;padding:9px 13px;color:#EF4444;font-size:13px;margin-bottom:14px}
  .filter-row{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:22px}
  .f-tab{padding:6px 16px;border-radius:99px;font-size:13px;font-weight:500;cursor:pointer;border:1px solid rgba(255,255,255,0.08);background:transparent;color:#6B7280;font-family:'DM Sans',sans-serif;transition:all 0.18s}
  .f-tab:hover{color:#E2E4EF;border-color:rgba(255,255,255,0.15)}
  .f-tab.on{background:rgba(108,99,255,0.15);border-color:rgba(108,99,255,0.35);color:#A78BFA}
  .cards-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(290px,1fr));gap:16px}
  .m-card{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:18px;overflow:hidden;transition:border-color 0.2s,transform 0.2s}
  .m-card:hover{border-color:rgba(108,99,255,0.3);transform:translateY(-3px)}
  .m-card-img{width:100%;height:120px;object-fit:cover;opacity:0.6;filter:saturate(0.7)}
  ...
  .m-name{font-weight:600;color:#fff;font-size:15px}
  .m-email{font-size:12px;color:#4B5563;margin-top:2px}
  .m-div{height:1px;background:rgba(255,255,255,0.05);margin:14px 20px}
  .m-skills{padding:0 20px}
  .m-sk-label{font-size:10px;font-weight:700;color:#4B5563;text-transform:uppercase;letter-spacing:0.9px;margin-bottom:7px}
  .tags{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px}
  .tag{font-size:12px;font-weight:500;padding:4px 11px;border-radius:99px}
  .tg-teach{background:rgba(108,99,255,0.13);color:#A78BFA;border:1px solid rgba(108,99,255,0.2)}
  .tg-learn{background:rgba(59,130,246,0.1);color:#60A5FA;border:1px solid rgba(59,130,246,0.18)}
  ...
  .empty{background:rgba(255,255,255,0.018);border:1px solid rgba(255,255,255,0.05);border-radius:18px;padding:64px 24px;text-align:center}
  .e-icon{font-size:44px;margin-bottom:14px}
  .e-title{font-family:'Syne',sans-serif;font-weight:700;color:#fff;font-size:16px;margin-bottom:5px}
  .e-sub{color:#4B5563;font-size:13px}
  .inbox-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(290px,1fr));gap:16px}
  .i-card{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:18px;padding:20px}
  .i-name{font-weight:600;color:#fff;font-size:15px}
  .i-email{font-size:12px;color:#4B5563;margin-top:2px;margin-bottom:10px}
  .i-skill{font-size:13px;color:#9CA3AF;margin-bottom:12px}
  .i-skill b{color:#A78BFA}
  .st-badge{display:inline-block;font-size:11px;font-weight:600;padding:3px 11px;border-radius:99px}
  .st-p{background:rgba(234,179,8,0.1);color:#EAB308;border:1px solid rgba(234,179,8,0.2)}
  .st-a{background:rgba(74,222,128,0.08);color:#4ADE80;border:1px solid rgba(74,222,128,0.18)}
  .st-r{background:rgba(239,68,68,0.08);color:#EF4444;border:1px solid rgba(239,68,68,0.18)}
  .act-row{display:flex;gap:8px;margin-top:14px}
  .btn-a{flex:1;padding:8px;border-radius:8px;background:rgba(74,222,128,0.09);border:1px solid rgba(74,222,128,0.2);color:#4ADE80;font-size:13px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif}
  .btn-a:hover{background:rgba(74,222,128,0.18)}
  .btn-r{flex:1;padding:8px;border-radius:8px;background:rgba(239,68,68,0.07);border:1px solid rgba(239,68,68,0.18);color:#EF4444;font-size:13px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif}
  .btn-r:hover{background:rgba(239,68,68,0.14)}
  .danger-zone{max-width:1280px;margin:48px auto 60px;padding:0 40px}
  .d-box{background:rgba(239,68,68,0.04);border:1px solid rgba(239,68,68,0.15);border-radius:16px;padding:24px 28px;display:flex;align-items:center;justify-content:space-between;gap:20px}
  .d-title{font-family:'Syne',sans-serif;font-weight:700;color:#EF4444;font-size:15px;margin-bottom:4px}
  .d-sub{font-size:13px;color:#6B7280}
  .d-btn{padding:10px 22px;border-radius:9px;background:transparent;border:1px solid rgba(239,68,68,0.4);color:#EF4444;font-family:'DM Sans',sans-serif;font-weight:600;font-size:14px;cursor:pointer;transition:all 0.2s;white-space:nowrap;flex-shrink:0}
  .d-btn:hover{background:rgba(239,68,68,0.1);border-color:#EF4444}
  .overlay{position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:999;display:flex;align-items:center;justify-content:center}
  .modal{background:#0C1120;border:1px solid rgba(239,68,68,0.25);border-radius:20px;padding:36px 32px;width:100%;max-width:420px}
  ...
  .m-cancel{flex:1;padding:11px;border-radius:9px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);color:#9CA3AF;font-family:'DM Sans',sans-serif;font-weight:600;font-size:14px;cursor:pointer}
  ...
  .ai-fab{position:fixed;bottom:28px;right:28px;z-index:500;width:56px;height:56px;border-radius:99px;background:linear-gradient(135deg,#6C63FF,#A78BFA);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:24px;box-shadow:0 4px 24px rgba(108,99,255,0.4);transition:transform 0.2s}
  .ai-fab:hover{transform:scale(1.1)}
  .ai-panel{position:fixed;bottom:96px;right:28px;z-index:500;width:360px;background:#0C1120;border:1px solid rgba(108,99,255,0.25);border-radius:20px;box-shadow:0 8px 40px rgba(0,0,0,0.6);display:flex;flex-direction:column;overflow:hidden;max-height:500px}
  .ai-head{padding:16px 18px;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;align-items:center;gap:10px}
  ...
  .ai-head-title{font-family:'Syne',sans-serif;font-weight:700;color:#fff;font-size:14px}
  .ai-head-sub{font-size:11px;color:#4ADE80;margin-top:1px}
  .ai-messages{flex:1;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:10px;min-height:200px}
  .ai-msg-user{align-self:flex-end;background:rgba(108,99,255,0.2);border:1px solid rgba(108,99,255,0.25);border-radius:14px 14px 4px 14px;padding:9px 13px;font-size:13px;color:#E2E4EF;max-width:85%}
  .ai-msg-bot{align-self:flex-start;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:4px 14px 14px 14px;padding:9px 13px;font-size:13px;color:#D1D5DB;max-width:85%;line-height:1.5}
  .ai-msg-typing{align-self:flex-start;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:4px 14px 14px 14px;padding:9px 13px}
  .ai-input-row{padding:12px 14px;border-top:1px solid rgba(255,255,255,0.06);display:flex;gap:8px}
  .ai-input{flex:1;padding:9px 13px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:10px;color:#E2E4EF;font-size:13px;font-family:'DM Sans',sans-serif;outline:none}
  .ai-input:focus{border-color:rgba(108,99,255,0.4)}
